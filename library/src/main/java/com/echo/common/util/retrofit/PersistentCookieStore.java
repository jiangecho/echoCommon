...
/**
 * Created by jiangecho on 15/12/31.
 */
...
public class PersistentCookieStore implements CookieStore {
    ...
    private static final String COOKIE_PREFS = "retrofit_CookiePrefsFile";
    ...
    public void add(URI uri, HttpCookie cookie) {
        ...
        if (!cookie.hasExpired()) {
            if (!cookies.containsKey(getHost(uri)))
                cookies.put(getHost(uri), new ConcurrentHashMap<String, HttpCookie>());
            cookies.get(getHost(uri)).put(name, cookie);
        } else {
            if (cookies.containsKey(uri.toString()))
                cookies.get(getHost(uri)).remove(name);
        }

        // Save cookie into persistent store
        SharedPreferences.Editor prefsWriter = cookiePrefs.edit();
        prefsWriter.putString(getHost(uri), TextUtils.join(",", cookies.get(getHost(uri)).keySet()));
    ...
    public List<HttpCookie> get(URI uri) {
        ...
        if (cookies.containsKey(getHost(uri)))
            ret.addAll(cookies.get(getHost(uri)).values());
        return ret;
    }
    ...
    public boolean remove(URI uri, HttpCookie cookie) {
        ...
        if (cookies.containsKey(getHost(uri)) && cookies.get(getHost(uri)).containsKey(name)) {
            cookies.get(getHost(uri)).remove(name);
